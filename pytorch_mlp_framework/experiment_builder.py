...
class ExperimentBuilder(nn.Module):
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        ########################################
         # Collect absolute mean of the gradients for each layer in all_grads with the layer names in layers
        for name, param in named_parameters:
            if param.requires_grad and "weight" in name:  # Exclude biases
                layers.append(name)
                all_grads.append(param.grad.abs().mean().item() if param.grad is not None else 0)
