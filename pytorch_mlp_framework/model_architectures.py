...
class ProcessingBlockBN(nn.Module):
    """
    Processing Block with Batch Normalization (BN).
    Combines the ConvolutionalProcessingBlock structure with BN after each convolution.
    """
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ProcessingBlockBN, self).__init__()
        self.input_shape = input_shape  # Accept and store input_shape
        self.num_filters = num_filters
        self.kernel_size = kernel_size
        self.padding = padding
        self.bias = bias
        self.dilation = dilation
        self.build_module()  # Ensure build_module is called in _init_

    def build_module(self):
        """
        Builds the module with two convolutional layers, each followed by Batch Normalization (BN).
        """
        self.layer_dict = nn.ModuleDict()
        x = torch.zeros(self.input_shape)  # Dummy tensor for shape inference
        out = x
        ...
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters,
                                              ...
                                              bias=self.bias, dilation=self.dilation)
        ...
        out = F.leaky_relu(self.layer_dict['bn_1'](out))

    def forward(self, x):
        """
        Forward pass through the Processing Block with BN.
        """
...
class ProcessingBlockBNRC(nn.Module):
    """
    Processing Block with Batch Normalization (BN) and Residual Connections (RC).
    Combines the ConvolutionalProcessingBlock structure with BN and RC.
    """
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ProcessingBlockBNRC, self).__init__()
        self.input_shape = input_shape  # Accept and store input_shape
        self.num_filters = num_filters
        self.kernel_size = kernel_size
        self.padding = padding
    ...
    def build_module(self):
        """
        Builds the module with two convolutional layers, each followed by Batch Normalization (BN),
        and includes a residual connection.
        """
        self.layer_dict = nn.ModuleDict()
        x = torch.zeros(self.input_shape)  # Dummy tensor for shape inference
        out = x
    ...
    def forward(self, x):
        """
        Forward pass through the Processing Block with BN and RC.
        """
        out = self.layer_dict['conv_0'](x)
        out = F.leaky_relu(self.layer_dict['bn_0'](out))
        ...
        out = F.leaky_relu(out + residual)  # Add the input to itself before final activation

        return out


class DownsamplingBlockBN(nn.Module):
    """
    Downsampling Block with Batch Normalization (BN).
    Combines the ConvolutionalProcessingBlock structure with BN, and includes a downsampling operation
    between convolutional layers.
    """
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor=2):
        super(DownsamplingBlockBN, self).__init__()
        self.input_shape = input_shape  # Accept and store input_shape
        self.num_filters = num_filters
        self.kernel_size = kernel_size
        self.padding = padding
    ...
    def build_module(self):
        """
        Builds the module with two convolutional layers, each followed by Batch Normalization (BN),
        and includes downsampling between the layers.
        """
        self.layer_dict = nn.ModuleDict()
        x = torch.zeros(self.input_shape)  # Dummy tensor for shape inference
        out = x
    ...
    def forward(self, x):
        """
        Forward pass through the Downsampling Block with BN.
        """
        out = self.layer_dict['conv_0'](x)
        out = F.leaky_relu(self.layer_dict['bn_0'](out))
