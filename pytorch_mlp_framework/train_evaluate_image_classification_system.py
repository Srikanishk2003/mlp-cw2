...
# Select the appropriate processing block and dimensionality reduction block based on the argument passed
if args.block_type == 'conv_block':
    ...
    processing_block_type = ProcessingBlockBN
    dim_reduction_block_type = DownsamplingBlockBN  # Use the original reduction block
elif args.block_type == 'bn_rc_block':
    processing_block_type = ProcessingBlockBNRC
    dim_reduction_block_type = DownsamplingBlockBN  # Use the original reduction block
else:
    raise ValueError(f"Unsupported block type: {args.block_type}")

# Create the network using the selected processing and dimensionality reduction blocks
...
# Initialize the ExperimentBuilder
